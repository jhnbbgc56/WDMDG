...
class VGG19(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        vgg_pretrained_features[0] = nn.Conv2d(12, 64, kernel_size=3, stride=1, padding=1)
...
def train(rank, gpu, args):
    ...
    if args.dataset in ['cifar10', 'stl10','CWRU']:
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        epoch_start_time = time.time()  # 开始记录这个 epoch 的时间
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            # 这个函数主要是用来生成一对扩散数据 (x_t, x_{t+1})
            x_t.requires_grad = True
            # 这是告诉
            # PyTorch，当计算图被构建时，需要为
            # x_t
            # 跟踪梯度。也就是说，PyTorch
            # 会记录该张量在计算过程中所涉及的操作，之后在反向传播（backward()）时计算相应的梯度。
            # train with real
...
#  这段代码实现了一个条件梯度惩罚的策略：
            #
            # 如果 args.lazy_reg 为 None，每一步都进行梯度惩罚。
            # 如果 args.lazy_reg 有值，比如 lazy_reg = 16，那么梯度惩罚只在每 16 个步数上执行一次。
            ...
            # detach() 的作用：当你不希望某个张量参与梯度计算时，可以使用 detach()。
            # 调用 detach() 后，返回的新张量将不会被记录在计算图中，因此后续的操作不会影响梯度的计算和反向传播
            ...
            if args.rec_loss:
                ...
                errG = errG + rec_loss
            if args.prectual_loss:
                vggloss=VGGLosses(0)
                vgg_loss,style_loss,perc_loss = vggloss(x_0_predict,real_data)
                errG = errG + 0.01*perc_loss
            errG.backward()
            optimizerG.step()
        ...
        if not args.no_lr_decay:
            ...
            schedulerG.step()
            schedulerD.step()
        epoch_end_time = time.time()  # 记录这个 epoch 结束的时间
        epoch_duration = epoch_end_time - epoch_start_time  # 计算这个 epoch 的总时间
        print(f"Epoch {epoch} training time: {epoch_duration:.2f} seconds")  # 打印这个 epoch 的总时间
        if rank == 0:
            if epoch % 10 == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', type=int, default=4,
                        help='num_workers')
    parser.add_argument("--prectual_loss",action="store_true")
